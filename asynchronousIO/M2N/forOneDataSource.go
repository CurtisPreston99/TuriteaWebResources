...
func (m *forOneDataSource) Load(key asynchronousIO.Key, source uint16) func() (asynchronousIO.Bean, error) {
	k, ok := key.UniqueId()
	if !ok {
	...
	if _, ok := bean.(asynchronousIO.Start); ok {
		go m.loadWorker(key, k, c, m.dataSource)
...
func (m *forOneDataSource) Save(bean asynchronousIO.Bean, source uint16) {
	key := bean.GetKey()
	k, ok := key.UniqueId()
	if !ok {
		panic("you must use a key whose UniqueId function returns a true")
	...
	lock.Unlock()
	if start {
		go m.saveWorker(nil, key, k, bean, m.dataSource)
	}
}

func (m *forOneDataSource) Delete(key asynchronousIO.Key, source uint16) func() error {
	k, ok := key.UniqueId()
	if !ok {
		panic("you must use a key whose UniqueId function returns a true")
	}
	...
	//fmt.Println("delete")
	if start {
		go m.deleteWorker(key, k, c, m.dataSource)
	}
	if c == nil {
		return func() (e error) {
			return nil
...
func (m *forOneDataSource) SaveAndCallBackWhenFinish(bean asynchronousIO.Bean, source uint16) func() error {
	key := bean.GetKey()
	k, ok := key.UniqueId()
	if !ok {
		panic("you must use a key whose UniqueId function returns a true")
	...
	if start {
		go m.saveWorker(c, key, k, bean, m.dataSource)
	}
	return genResult(c, m.channelBuffer).error
}

func newForOneDataSource(source asynchronousIO.DataSource, beanTypeNumber int64, expectBufferNumber int64) asynchronousIO.AsynchronousIOMachine {
	...
	return &forOneDataSource{inUse: table, channelBuffer: pool, dataSource: source, bufferBlockNumber: blockNumber / beanTypeNumber, locks: locks}
}

func (m *forOneDataSource) loadWorker(key asynchronousIO.Key, k int64, c chan asynchronousIO.Bean, dataSource asynchronousIO.DataSource) {
	p := k%m.bufferBlockNumber + m.bufferBlockNumber*key.TypeId()
	lock := m.locks[p]
	table := m.inUse[p]
	cmd := Load
...
func (m *forOneDataSource) saveWorker(c chan asynchronousIO.Bean, key asynchronousIO.Key, k int64, bean asynchronousIO.Bean, dataSource asynchronousIO.DataSource) {
	p := k%m.bufferBlockNumber + m.bufferBlockNumber*key.TypeId()
	lock := m.locks[p]
	table := m.inUse[p]
	cmd := ToSave
...
func (m *forOneDataSource) deleteWorker(key asynchronousIO.Key, k int64, c chan asynchronousIO.Bean, dataSource asynchronousIO.DataSource) {
	p := k%m.bufferBlockNumber + m.bufferBlockNumber*key.TypeId()
	err := dataSource.Delete(key)
	if err != nil {
		c <- asynchronousIO.AsynchronousIOError{err}
