...
function loadUsers() {
    $.get("../api/allUser", function (r) {
        ...
        for (let i = 0; i < length; i++) {
            let row = `<tr class="user"><td><input type="checkbox" name="selectUsers" value="{0}"></td><td>{0}</td><td><button id="{0}" onclick="changeRole('{0}',{2})" >{1}</button></td></tr>`.format(names[i], roles[i] === 1 ? "Researcher" : (roles[i] === 2 ? "Administer" : ""), roles[i]);
            list.append($(row));
        }
    }).fail(function () {
